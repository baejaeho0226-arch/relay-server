...
async function RequestHandler(req, res) {
    ...
    if (pathname === '/api/login' && method === 'POST') {
        ...
        if (!result.ok) {
            ApiError(res, result.status || 401, result.code, result.retryAt || '');
            return;
        }
...
function StartWebAdmin() {
    ...
    server.listen(port, config.HOST, () => {
        ...
        if (config.ADMIN_CREDENTIALS.admin === 'ADMIN-SECRET-KEY-1234' && String(process.env.WEB_ADMIN_ALLOW_DEFAULT_SECRET || '') !== '1') {
            console.log('Web Admin login disabled for default ADMIN_SECRET. Set ADMIN_SECRET in Railway Variables.');
        }
    });
    return server;
}
