...
function Execute(c,requestId,action,body={}){
 ...
 const read={follows:()=>require('./follows').List(p,body),charge:()=>require('./charges').Read(p),product:()=>commerce.Product(body,p),article:()=>social.Article(p,body),home:()=>({profile:s.PublicProfile(p,true),news:social.News(p,body),settings:{}}),news:()=>social.News(p,body),catalog:()=>({...commerce.Catalog(body),profile:s.PublicProfile(p,true)}),me:()=>commerce.Mine(p,body),feed:()=>social.Feed(p,body),thread:()=>social.Thread(p,body)};
