...
function Execute(c,requestId,action,body={}){
 ...
 const result=s.Operation(p,requestId,action,body,()=>{const result=mutations[action]();return body._delta?require('./wire').Compact(result,action):result;},action==='order.activate'?['licenses','licenseRevision']:[]);
