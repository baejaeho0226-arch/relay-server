...
function Execute(c,requestId,action,body={}){
 ...
 const read={member:()=>require('./profiles').Read(p,body),follows:()=>require('./follows').List(p,body),charge:()=>require('./charges').Read(p),product:()=>commerce.Product(body,p),article:()=>social.Article(p,body),home:()=>require('./identity').Home(p),records:()=>{if(body.handle&&s.Resolve(body.handle)?.id!==p.id)s.Fail('NOT_OWNER');return require('./identity').Read(p,body,false);},news:()=>social.News(p,body),catalog:()=>({...commerce.Catalog(body),profile:s.PublicProfile(p,true)}),me:()=>commerce.Mine(p,body),feed:()=>social.Feed(p,body),thread:()=>social.Thread(p,body)};
 if(read[action])return {...read[action](),viewer:s.PublicProfile(p)};
 const mutations={
