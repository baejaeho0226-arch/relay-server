...
function CurrentWallet(p,data,body={},includeWallet=false){
 // Copy only response projections. Receipts (reward, spin, conversion, order and
 ...
 if(data.rewards)result.rewards=CurrentWallet(p,data.rewards,{limit:3});
 return result;
}
function Execute(c,requestId,action,body={}){
 ...
 const result=s.Operation(p,requestId,action,body,()=>{const beforePoints=p.points||0,context=badges.Before(p,action,body);const result=mutations[action]();badges.After(p,action,body,result,context);return CurrentWallet(p,body._delta?require('./wire').Compact(result,action):result,body,(p.points||0)!==beforePoints);},action==='order.activate'?['licenses','licenseRevision']:[]);
 ...
 return CurrentWallet(s.ProfileById(p.id),result,body);
}
...
function Handle(c,line){
 ...
 try{
  ...
  if(!['test.enter','history.record'].includes(action)&&(action.includes('.')||['purchase','react','report'].includes(action)))NotifyChanged(c);
