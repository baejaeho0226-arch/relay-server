...
function Execute(c,requestId,action,body={}){
 ...
 const result=s.Operation(p,requestId,action,body,()=>{const context=badges.Before(p,action,body);const result=mutations[action]();badges.After(p,action,body,result,context);return body._delta?require('./wire').Compact(result,action):result;},action==='order.activate'?['licenses','licenseRevision']:[]);
...
function Handle(c,line){
 ...
 try{
  ...
  if(action!=='test.enter'&&(action.includes('.')||['purchase','react','report'].includes(action)))NotifyChanged(c);
