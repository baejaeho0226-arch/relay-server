...
function Execute(c,requestId,action,body={}){
 if(action==='test.enter'){
  ...
  return {testAccess:true,memberProtocol:33};
 }
 ...
 const read={policies:()=>require('./documents').Read(body),preferences:()=>({preferences:require('./preferences').Read(p),profile:s.PublicProfile(p,true)}),live:()=>require('./live').Read(p,body),gif:()=>{const post=require('./socialActions').Post(p,body.id);return {photo:{id:post.id,gif:require('./gifMedia').Public(post,true)}};},photo:()=>{const post=require('./socialActions').Post(p,body.id);return {photo:{id:post.id,image:post.image||''}};},gifs:()=>require('./gifs').List(),bookmarks:()=>require('./socialActions').Bookmarks(p,body),blocks:()=>require('./socialActions').Blocks(p,body),member:()=>require('./profiles').Read(p,body),follows:()=>require('./follows').List(p,body),charge:()=>require('./charges').Read(p),product:()=>commerce.Product(body,p),article:()=>social.Article(p,body),home:()=>require('./identity').Home(p),news:()=>social.News(p,body),catalog:()=>({...commerce.Catalog(body,p),profile:s.PublicProfile(p,true)}),me:()=>commerce.Mine(p,body),feed:()=>social.Feed(p,body),thread:()=>social.Thread(p,body)};
 if(read[action]){
  // Validate visibility (and explicit detail opens) before accepting a cached revision.
  const data=read[action]();
  if(action==='live')return {...data,memberProtocol:33};
  if(['feed','thread','bookmarks'].includes(action)&&body._since===s.DB().revision)return {unchanged:true,revision:s.DB().revision,memberProtocol:33};
  return {...data,viewer:s.PublicProfile(p),memberProtocol:33,revision:s.DB().revision};
 }
 const mutations={
  'attendance.check':()=>require('./home').Check(p),
