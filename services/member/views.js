...
function Article(p,row){
 const day=new Date().toISOString().slice(0,10),key='news:'+row.id,hit=p.id+':'+key;
 if(s.DB().viewHits[hit]===day)return;
 s.Atomic(()=>{const db=s.DB();db.viewHits[hit]=day;db.viewCounters[key]={kind:'news',id:row.id,count:s.ViewCount('news',row.id)+1,updatedAt:Date.now()};});
}
