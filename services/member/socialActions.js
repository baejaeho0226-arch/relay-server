...
function Visible(post,p){return !!post&&!post.deleted&&!post.hidden&&!s.ProfileById(post.accountId)?.blocked&&(!p||!Blocked(p.id,post.accountId));}
function Post(p,id){const row=s.DB().posts[id];if(!Visible(row,p))s.Fail('POST_NOT_FOUND');return row;}
function Comment(p,id){const row=s.DB().comments[id];if(!row||row.deleted||row.hidden||Blocked(p.id,row.accountId)||s.ProfileById(row.accountId)?.blocked)s.Fail('COMMENT_NOT_FOUND');Post(p,row.postId);return row;}
...
function PublicComment(row,p,counts){counts=counts||require('./commentThreads').Counts(row.postId,p);const post=Post(p,row.postId),reactions=Object.values(s.DB().commentReactions).filter(x=>x.commentId===row.id);return {id:row.id,postId:row.postId,body:row.body,at:row.at,updatedAt:row.updatedAt||row.at,revision:row.revision||0,parentId:row.parentId||'',replyToId:require('./commentThreads').Parent(row,counts.parents),replies:counts.get(row.id)||0,replyTo:row.replyTo||'',replyToName:row.replyTo?require('./social').Author(row.replyTo).nickname:'',replyToHandle:row.replyTo?require('./social').Author(row.replyTo).handle:'',author:require('./social').Author(row.accountId),own:row.accountId===p.id,isPostAuthor:row.accountId===post.accountId,likes:reactions.length,myReaction:s.DB().commentReactions[p.id+':'+row.id]?1:0,bookmarked:Bookmarked(p,'comment',row.id)};}
...
function CommentEdit(p,body){const row=Comment(p,body.id);if(row.accountId!==p.id)s.Fail('NOT_OWNER');if(body.revision!==(row.revision||0))s.Fail('CONTENT_CHANGED');row.body=s.Text(body.body,600,true);row.updatedAt=Date.now();row.revision=(row.revision||0)+1;return {comment:PublicComment(row,p)};}
