...
function Post(p,body){Rate(p,'post',10000);const id=s.Id('POST'),post={id,accountId:p.id,title:s.Text(body.title,90),poll:extra.PollInput(body.poll),...require('./gifMedia').Fields(body),...require('../../public/member-body-format').prepare(body.body,body.bodyFormats),...require('./media').PostFields(body.image),imagePosition:require('./media').PostPosition(body.imagePosition),at:Date.now(),deleted:false,hidden:false};if(!post.title&&!post.body&&!post.image&&!post.gifId&&!post.poll)s.Fail('INPUT_INVALID');s.DB().posts[id]=post;return {post:PublicPost(post,p,false,body._wire==='zlib')};}
