...
function PublicProfile(p,own=false){
 const metrics={posts:Object.values(DB().posts).filter(x=>x.accountId===p.id&&!x.deleted&&!x.hidden).length,...require('./follows').Counts(p.id)};
 return {id:p.id,handle:Handle(p),nickname:p.nickname,nicknameColor:p.nicknameColor||'',titleBadge:require('./badges').Public(p,metrics),bio:p.bio,pronouns:p.pronouns||'',avatar:own?p.avatar:PublicAvatar(p),avatarRevision:p.avatarRevision,profileRevision:p.profileRevision||p.avatarRevision||0,...metrics,...(own?{balance:p.balance,points:p.points||0,eventSpins:p.eventSpins||0,inventory:require('./customization').Inventory(p),createdAt:p.createdAt,handleEditable:!p.handleChangedAt,nicknameChangeAt:p.nicknameChangedAt?p.nicknameChangedAt+30*86400000:0,gender:p.gender||'UNDISCLOSED',preferences:require('./preferences').Read(p)}: {})};
}
