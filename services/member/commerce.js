...
function Product(body,p){const row=s.DB().products[body.id];if(!row||!row.published||row.deleted)s.Fail('PRODUCT_UNAVAILABLE');if(p)require('./views').Article(p,row,'product');return {product:PublicGame(row,true),...(p?{profile:s.PublicProfile(p,true)}:{})};}
