...
function List(p,body){
 const followers=body.mode==='followers',target=(body.handle||body.id)?s.Resolve(body.handle||body.id):p;if(!target||target.blocked)s.Fail('MEMBER_NOT_FOUND');
 const rows=Object.values(s.DB().follows).filter(x=>followers?x.following===target.id:x.follower===target.id).sort((a,b)=>b.at-a.at).map(x=>s.ProfileById(followers?x.follower:x.following)).filter(x=>x&&!x.blocked);
