...
(async()=>{try{
 ...
 const list=await run(viewer,'news',{summary:true});assert.equal(list.items[0].body,undefined);assert.equal(list.items[0].image,undefined);assert.equal(list.items[0].views,undefined);assert.ok(list.items[0].imageThumb.startsWith('data:image/jpeg;'));assert.equal(list.viewer.id,b.id);
