...
(async()=>{try{
 ...
 const product=(await run(viewer,'product',{id:game.id})).product;assert.ok(product.image.length>12000,'photo detail uses signed chunk transport');assert.deepEqual(product.details,gameDetails);assert.equal((await run(viewer,'catalog')).items[0].image,undefined);assert.ok(product.imageThumb.length<=16100);
 const preserved=save('product.save',{id:game.id,revision:game.revision,title:'제목 수정',description:'',accessType:'TYPE1',published:true});assert.deepEqual(store.DB().products[game.id].details,gameDetails);assert.equal(store.DB().products[game.id].image,product.image);
 ...
 assert.throws(()=>save('product.save',{...store.DB().products[game.id],details:{channels:{official:'javascript:alert(1)'}}}),/CONTENT_URL_INVALID/);
 ...
 const snapshot=db.BuildDatabaseObject();assert.equal(db.ImportDatabaseObject(snapshot),true);assert.equal(store.DB().products[game.id].image,product.image);assert.deepEqual(store.DB().products[game.id].details,gameDetails);
