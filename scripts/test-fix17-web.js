...
(async()=>{try{
 ...
 const store=require('../services/member/store');const products=Object.values(store.DB().products);assert.equal(products.length,1);assert.deepEqual(products[0].plans.map(x=>x.days),[1,7,15,30]);assert.equal(products[0].plans[3].price,20000);
 ...
 chargeInput.dispatchEvent(new w.Event('change',{bubbles:true}));await wait();assert.equal(w.memberCanAutoRefresh(),false);assert.ok(w.document.querySelector('#member-charge-preview').src.startsWith('data:image/png;base64,'));
 await w.renderMember();assert.ok(w.document.querySelector('#member-charge-preview').src.startsWith('data:image/png;base64,'));
 ...
 assert.ok(w.document.getElementById('content').textContent.includes('잔액 충전'));assert.equal(w.document.querySelector('[data-member-action="charge.reject"]'),null);assert.ok(w.document.querySelector('#member-charge-preview').classList.contains('hidden'));
 ...
 console.log('FIX17 ADMIN DOM PASS: 10 wrapped tabs, feed-only counts, owner/admin edits, empty pagination, search/archive/restore, safe automatic refresh, unchanged DOM and scroll preservation, retired coin routes, four duration price forms, QR preview retention and actual QR wallet approval');
