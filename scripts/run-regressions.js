...
const files = fs.readdirSync(__dirname).filter(name => /^test-.*\.js$/.test(name) &&
  (!process.argv.includes('--fix12') || name.startsWith('test-fix12-')) && (!process.argv.includes('--fix13') || name.startsWith('test-fix13-')) && (!process.argv.includes('--fix17') || name.startsWith('test-fix17-'))).sort();
let failed = 0;
