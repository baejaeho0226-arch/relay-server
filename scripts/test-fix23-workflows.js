...
(async()=>{try{
 ...
 const detail=hub.AdminRead({view:'products',id:game.id}).items[0];assert.equal(detail.genre,'레이싱');assert.equal(detail.details,undefined);assert.equal(detail.image,undefined);assert.deepEqual(store.DB().products[game.id].details,legacyDetails);
 w.switchView('member-products');await w.renderMember();const gameIcon=content.querySelector('.member-game-icon');assert.ok(gameIcon);assert.equal(gameIcon.getAttribute('aria-label'),'레이싱');assert.equal(gameIcon.textContent,'');assert.ok(gameIcon.querySelector('svg'));assert.equal(content.querySelector('.member-category-label'),null);assert.equal(content.querySelector('[data-category-tone]'),null);await click(content.querySelector('[data-member-action="product.edit"]'));assert.equal(w.document.querySelector('[data-modal-preview="image"]'),null);assert.equal(w.document.querySelector('[data-modal-field="genre"]').value,'레이싱');assert.equal(w.document.querySelector('[data-modal-field="minimum_cpu"]'),null);await click(w.document.querySelector('#modal-cancel'));assert.equal(store.DB().products[game.id].image,image);
