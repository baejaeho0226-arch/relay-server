...
(async()=>{try{
 ...
 assert.equal(w.document.querySelector('[data-modal-image],[data-modal-preview],.modal-section'),null,'game editor omits retired media and information');
 ...
 const s=require('../services/member/store'),game=Object.values(s.DB().products)[0];assert.equal(game.image,undefined);assert.equal(game.details,undefined);assert.equal(game.genre,'RPG');assert.deepEqual(game.plans,[{days:45,price:4200},{days:90,price:6000}]);
 ...
 assert.deepEqual(errors,[]);console.log('FIX25 WEB DOM PASS: CSP-safe feed FileReader upload, decoded preview/expand, text-only game genre selection and private legacy data, arbitrary plans add/remove, duplicate prevention before closing, persistence and edit reload.');
