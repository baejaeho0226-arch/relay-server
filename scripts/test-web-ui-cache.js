...
function testBundle() {
  ...
  try {
    ...
    fs.writeFileSync(path.join(temp, 'admin.js'), source('public/admin.js').replace("'fix11'", "'biometric1'"));
...
async function testRefresh(ready) {
  ...
  const context = {
    ...
    fetch: async (_, options) => {
      ...
      return { ok: true, json: async () => ({ ready, uiRevision: 'fix11', issues: ['public/admin.js'] }) };
    }
  };
  ...
  if (ready) {
    ...
    assert.match(redirected, /^\/\?ui-refresh=fix11-/);
  } else {
...
async function testWorker() {
  ...
  vm.runInNewContext(source('public/service-worker.js'), {
    ...
    caches: { open: async () => cache, keys: async () => ['relay-admin-shell-old', 'relay-admin-shell-v4.0.0-fix11', 'another-app-cache'], delete: async name => { removed.push(name); } },
    fetch: async request => {
  ...
  await request('/admin.js?v=4.0.0-fix11');
