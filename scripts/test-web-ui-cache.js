...
function testBundle() {
  ...
  try {
    ...
    fs.writeFileSync(path.join(temp, 'admin.js'), source('public/admin.js').replace("'fix21'", "'biometric1'"));
...
async function testRefresh(ready) {
  ...
  const context = {
    ...
    fetch: async (_, options) => {
      ...
      return { ok: true, json: async () => ({ ready, uiRevision: 'fix21', issues: ['public/admin.js'] }) };
    }
  };
  ...
  if (ready) {
    ...
    assert.match(redirected, /^\/\?ui-refresh=fix21-/);
  } else {
...
async function testWorker() {
  ...
  vm.runInNewContext(source('public/service-worker.js'), {
    ...
    caches: { open: async () => cache, keys: async () => ['relay-admin-shell-old', 'relay-admin-shell-v4.10.0-fix21', 'another-app-cache'], delete: async name => { removed.push(name); } },
    fetch: async request => {
  ...
  await request('/admin.js?v=4.10.0-fix21');
