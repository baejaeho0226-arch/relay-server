...
(async()=>{try{
 ...
 let home=await run(a,'home');assert.equal(home.recentOrders.length,1);assert.equal(home.recentOrders[0].id,order2.id);assert.equal(home.recentPayments.length,1);assert.notEqual(home.recentPayments[0].kind,'QR_TOPUP');
 store.Atomic(()=>{const x=store.DB().orders[order2.id];x.activatedAt=Date.now()-500;x.status='ACTIVE';});
 home=await run(a,'home');assert.equal(home.recentOrders[0].id,order2.id,'latest used pass precedes unused pass');assert.equal(home.games.items[0].id,game.id);
