...
(async()=>{try{
 ...
 for(const view of ['overview','news','products','profiles','posts','comments','reports','orders','ledger','charges']){
 ...
 // The administrator scans the actual member QR image and registers one game pass.
 ...
 const png=await require('qrcode').toBuffer('RCH1.'+charge.id+'.'+charge.token,{errorCorrectionLevel:'H',scale:6});
 await click(w.document.querySelector('[data-member-view="charges"]'));
 const chargeInput=w.document.querySelector('#member-charge-file');Object.defineProperty(chargeInput,'files',{configurable:true,value:[new w.File([png],'charge.png',{type:'image/png'})]});
 assert.equal(w.memberCanAutoRefresh(),false);
 await click(w.document.querySelector('[data-member-action="charge.scan"]'));
 assert.ok(w.document.getElementById('modal').textContent.includes(profile.nickname));field('amount','20000');field('days','45');field('accessType','TYPE3');field('memo','입금 확인 후 등록');
 await click(w.document.getElementById('modal-confirm'));assert.equal(store.DB().chargeRequests[charge.id].status,'APPROVED');
 const order=store.DB().orders[store.DB().chargeRequests[charge.id].orderId];assert.equal(order.accountId,profile.id);assert.equal(order.amount,20000);assert.equal(order.days,45);assert.equal(order.accessType,'TYPE3');assert.equal(order.activatedAt,0);
 assert.ok(w.document.getElementById('content').textContent.includes('로스트사가'));assert.equal(w.document.querySelector('[data-member-action="charge.reject"]'),null);
 await click(w.document.querySelector('[data-member-view="products"]'));await click(w.document.querySelector('[data-member-action="product.edit"]'));
 assert.equal(w.document.querySelector('[data-modal-field="price"]'),null);assert.equal(w.document.querySelector('[data-modal-field="days"]'),null);await click(w.document.getElementById('modal-confirm'));
 ...
 console.log('FIX16 ADMIN DOM PASS: 10 wrapped tabs, feed-only counts, owner/admin edits, empty pagination, search/archive/restore, safe automatic refresh, unchanged DOM and scroll preservation, retired coin routes, read-only game forms and actual QR charge photo approval');
