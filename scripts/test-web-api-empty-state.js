...
async function run() {
    ...
    assert.ok(html.includes('/admin-pages-support.js?v=4.10.0-fix21'));
    ...
    assert.deepStrictEqual(await ui.json(), { ready: true, webAdminVersion: '4.10.0', uiRevision: 'fix21', issues: [] });
    ...
    for (const file of ['admin-pages-support.js', 'admin.css', 'service-worker.js']) {
        const asset = await fetch(`http://127.0.0.1:${webPort}/${file}?v=4.10.0-fix21`);
