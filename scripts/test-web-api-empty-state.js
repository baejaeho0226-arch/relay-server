...
async function run() {
    ...
    assert.ok(html.includes('/admin-pages-support.js?v=4.4.0-fix15'));
    ...
    assert.deepStrictEqual(await ui.json(), { ready: true, webAdminVersion: '4.4.0', uiRevision: 'fix15', issues: [] });
    ...
    for (const file of ['admin-pages-support.js', 'admin.css', 'service-worker.js']) {
        const asset = await fetch(`http://127.0.0.1:${webPort}/${file}?v=4.4.0-fix15`);
