...
async function run() {
    ...
    assert.ok(html.includes('/admin-pages-support.js?v=3.5.4-fix9'));
    ...
    assert.deepStrictEqual(await ui.json(), { ready: true, webAdminVersion: '3.5.4', uiRevision: 'fix9', issues: [] });
    ...
    for (const file of ['admin-pages-support.js', 'admin.css', 'service-worker.js']) {
        const asset = await fetch(`http://127.0.0.1:${webPort}/${file}?v=3.5.4-fix9`);
