...
async function run() {
    ...
    assert.ok(html.includes('/admin-pages-support.js?v=4.11.0-fix22'));
    ...
    assert.deepStrictEqual(await ui.json(), { ready: true, webAdminVersion: '4.11.0', uiRevision: 'fix22', issues: [] });
    ...
    for (const file of ['admin-pages-support.js', 'admin.css', 'service-worker.js']) {
        const asset = await fetch(`http://127.0.0.1:${webPort}/${file}?v=4.11.0-fix22`);
