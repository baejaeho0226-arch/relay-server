...
async function run() {
    ...
    assert.ok(html.includes('/admin-pages-support.js?v=4.22.0-fix35'));
    ...
    assert.deepStrictEqual(await ui.json(), { ready: true, webAdminVersion: '4.22.0', uiRevision: 'fix35', issues: [] });
    ...
    for (const file of ['admin-pages-support.js', 'admin.css', 'service-worker.js']) {
        const asset = await fetch(`http://127.0.0.1:${webPort}/${file}?v=4.22.0-fix35`);
