...
(async()=>{try{
 ...
 assert.equal(w.document.querySelector('[data-modal-field="detail_genre"]'),null,'news has no game information');
 ...
 const storedNews=Object.values(store.DB().news)[0];store.Atomic(()=>{storedNews.image=testPhoto;storedNews.imageThumb=testPhoto;storedNews.details={platform:'legacy'};});
 ...
 field('body','사진 없이 수정한 소식');await click(w.document.getElementById('modal-confirm'));
 assert.equal(store.DB().news[storedNews.id].body,'사진 없이 수정한 소식');assert.equal(store.DB().news[storedNews.id].image,testPhoto,'editing text preserves legacy media in storage only');
