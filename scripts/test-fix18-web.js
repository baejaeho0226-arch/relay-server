...
(async()=>{try{
 ...
 assert.equal(w.document.querySelector('[data-modal-image="image"]'),null,'game editor has no photo input');
 ...
 const store=require('../services/member/store');const products=Object.values(store.DB().products);assert.equal(products.length,1);assert.deepEqual(products[0].plans.map(x=>x.days),[1,7,15,30]);assert.equal(products[0].plans[3].price,20000);assert.equal(products[0].genre,'레이싱');assert.equal(products[0].image,undefined);assert.equal(products[0].details,undefined);
 const legacyDetails={genre:'레이싱',developer:'테스트 제작사',platform:'legacy'};store.Atomic(()=>{Object.assign(products[0],{image:testPhoto,imageThumb:testPhoto,details:legacyDetails});});await w.renderMember();assert.equal(w.document.querySelector('.member-content-thumb'),null,'retired game media is not rendered');
