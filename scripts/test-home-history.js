...
(async()=>{try{
 ...
 assert.deepEqual((await run(a2,'home')).recentServices.map(x=>x.route),['settings','support']);
 assert.deepEqual((await run(b,'home')).recentServices,[]);
 ...
 for(let i=0;i<3;i++){await run(a,'home');await run(a,'history.record',{route:'settings'});}
 assert.deepEqual(history.Read(store.ProfileById(pa.id)),before);
 assert.equal(store.DB().revision,revision,'home refresh and repeated newest navigation are read-only');
 const routes=Object.keys(history.SERVICES).slice(0,25);
 for(const route of routes)await run(a,'history.record',{route});
 const services=(await run(a,'home')).recentServices;
 ...
 const moved=(await run(a,'home')).recentServices;assert.equal(moved[0].route,services[5].route);assert.equal(new Set(moved.map(x=>x.route)).size,20);
 const products=[];
 ...
 assert.deepEqual((await run(a,'home')).recentProducts,[],'previews and implicit reads never add history');
 for(const product of products)await run(a,'product',{id:product.id,countView:true});
 let viewed=(await run(a2,'home')).recentProducts;
 ...
 assert.deepEqual((await run(a,'home')).recentProducts,opened,'refresh or repeated newest detail cannot change recency');
 await run(a,'product',{id:products[5].id,countView:true});viewed=(await run(a,'home')).recentProducts;
 ...
 viewed=(await run(a,'home')).recentProducts;assert.ok(viewed.every(x=>![products[5].id,products[22].id].includes(x.id)));
 assert.deepEqual((await run(b,'home')).recentProducts,[]);
