...
(async()=>{try{
 ...
 const rewards=require('../services/member/rewards');assert.equal(rewards.Rules().attendanceDays,5);assert.equal(rewards.Rules().attendancePoints,80);assert.equal(rewards.Rules().chargeUnit,2000);assert.equal(rewards.Rules().prizes[0].points,25);assert.deepEqual(rewards.Rules().pointExchange,{enabled:true,cashUnit:200,pointUnit:3});assert.match(w.document.querySelector('.member-shell').textContent,/200원 ↔ 3 P/);
