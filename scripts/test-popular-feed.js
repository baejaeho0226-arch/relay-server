...
try{
 ...
 let popular=run(viewer,'popular',{_wire:'zlib'}),home=run(viewer,'home');
 assert.equal(popular.items.length,10);assert.equal(popular.total,10);assert.equal(popular.nextOffset,null);
 assert.deepEqual(popular.items.map(x=>x.id),home.popular.map(x=>x.id),'preview and full collection have identical popular order');
 ...
 assert.deepEqual(run(viewer,'popular',{limit:1,offset:20,mine:true,following:true,sort:'latest'}).items.map(x=>x.id),home.popular.map(x=>x.id),'fixed Top10 ignores ordinary feed paging/filter overrides');
 ...
 popular=run(viewer,'popular',{_since:revision});assert.equal(popular.unchanged,undefined);assert.equal(popular.items[0].id,promoted.id);assert.deepEqual(popular.items.map(x=>x.id),run(viewer,'home').popular.map(x=>x.id));
 ...
 run(viewer,'block.set',{id:authorId,blocked:true});assert.ok(run(viewer,'popular').items.every(x=>x.author.id!==authorId));assert.deepEqual(run(viewer,'popular').items.map(x=>x.id),run(viewer,'home').popular.map(x=>x.id));
 ...
 console.log('POPULAR FEED PASS: full Top10 cards, identical preview order, media/viewer state, live rank changes, edit/delete/block visibility, signed updates and no background views.');
