...
async function renderProcessors() {
  ...
  <div class="section-card"><div class="section-head"><h3>Type Processor Routing</h3><span class="small-note">TYPE IS VERIFIED BY BUILD SESSION</span></div><div class="table-wrap"><table><thead><tr><th>APK Type</th><th>WinSockServer Processor</th><th>Trust Source</th></tr></thead><tbody>${(processors.routes||[]).map(x=>`<tr><td>${badge(x.accessType)}</td><td class="code">${esc(x.processor)}</td><td>HMAC Build Session</td></tr>`).join('')}</tbody></table></div></div>
...
async function renderBuildSessions() {
  ...
  <div class="section-card"><div class="section-head"><h3>Build Session History</h3><span class="small-note">PENDING / AUTHORIZED / EXPIRED / REVOKED / FAILED</span></div><div class="table-wrap"><table><thead><tr><th>Status</th><th>Session</th><th>Client</th><th>Server</th><th>Type</th><th>Request</th><th>Authorized</th><th>Expires</th><th>Reason</th><th>Action</th></tr></thead><tbody>${sessionRows || '<tr><td colspan="10" class="empty">Build Session 없음</td></tr>'}</tbody></table></div></div>`;
}
...
async function renderQrAuth() {
  ...
  <div class="section-card"><div class="section-head"><h3>QR 인증 요청 이력</h3><span class="small-note">승인·거절·만료 감사 추적</span></div><div class="table-wrap"><table><thead><tr><th>Status</th><th>Request</th><th>Client</th><th>Type</th><th>Issued</th><th>Expires</th><th>Operator</th><th>Reason</th><th>Action</th></tr></thead><tbody>${rows || '<tr><td colspan="9" class="empty">QR 인증 요청 없음</td></tr>'}</tbody></table></div></div>`;
