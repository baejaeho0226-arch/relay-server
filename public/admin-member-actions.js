...
async function handleMemberAction(event){
 ...
 if(action==='product.new'||action==='product.edit'){
  ...
  values=await openModal({title:'본문 수정',fields:[{name:'body',label:'내용',type:'textarea',value:row.body},...(action==='post.edit'?[{name:'image',label:'게시물 사진',type:'image',value:row.image}]:[])],confirmLabel:'저장'});if(values)body={...values,action:action==='post.edit'?'post.save':'comment.save',id:row.id,revision:row.revision||0};
