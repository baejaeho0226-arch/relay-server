...
async function handleMemberAction(event){
 ...
 if(action==='product.new'||action==='product.edit'){
  ...
  values=await openModal({title:'본문 수정',fields:[...(action==='post.edit'?[{name:'title',label:'제목',value:row.title||''}]:[]),{name:'body',label:'내용',type:action==='post.edit'?'richtext':'textarea',value:row.body,formats:row.bodyFormats||[]},...(action==='post.edit'?[{name:'image',label:'게시물 사진',type:'image',value:row.image},{name:'imagePosition',label:'글과 사진 배치',type:'select',value:row.imagePosition||'after',options:[{value:'after',label:'사진 위에 글'},{value:'before',label:'사진 아래에 글'}]}]:[])],confirmLabel:'저장'});if(values)body={...values,action:action==='post.edit'?'post.save':'comment.save',id:row.id,revision:row.revision||0};
