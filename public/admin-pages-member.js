...
let memberPointerDown=false,memberInteractionUntil=0,memberLastRefresh=0,memberFingerprint='';
const memberTabs={overview:'운영 요약',news:'소식',products:'쇼핑',profiles:'회원',posts:'피드 글',comments:'댓글',reports:'신고',orders:'구매 내역',ledger:'결제 원장'};
const memberStatus={PAID:'구매 완료',ACTIVE:'사용 중',REFUNDED:'환불 완료',TOPUP:'이전 잔액 반영',PURCHASE:'구매',REFUND:'환불',OPEN:'접수',RESOLVED:'처리 완료',UPDATE:'업데이트',NOTICE:'공지',EVENT:'이벤트',ALERT:'알림'};
...
function memberRow(row,view){
 ...
 if(view==='products'){meta=`${accessTypeName(row.accessType)} · ${row.days}일 ${memberMoney(row.price)} · 재고 ${row.stock===-1?'무제한':row.stock}`;body=row.description;actions=memberButton('product.edit',row.id,'수정');}
 if(view==='news'){meta=`${memberStatus[row.category]}${row.pinned?' · 상단 고정':''}`;body=row.body;actions=memberButton('news.edit',row.id,'수정');}
 if(view==='orders'){meta=`${row.accountId} · ${memberMoney(row.amount)}`;body=row.activatedAt?'사용 시작 '+fmtTime(row.activatedAt):'미사용';if(!row.activatedAt&&row.status==='PAID')actions=memberButton('order.refund',row.id,'잔액 환불',true);}
 if(view==='ledger'){title=memberStatus[row.kind]||'거래';meta=`${row.accountId} · ${memberMoney(row.amount)} · 잔액 ${memberMoney(row.balance)}`;body=row.reference;}
...
function memberCanAutoRefresh(){
 ...
 return currentView==='member'&&!document.hidden&&!memberPointerDown&&Date.now()>=memberInteractionUntil&&memberSelected.size===0&&
...
async function renderMember(automatic=false){
 ...
 if(view==='overview'){
  const stats=[['회원',result.members],['판매 상품',result.products],['구매',result.orders],['피드 조회수',result.postViews],['미처리 신고',result.openReports]];
  ...
  let add='';if(view==='products')add=memberButton('product.new','','상품 등록');if(view==='news')add=memberButton('news.new','','소식 작성');
...
content.addEventListener('pointerdown',()=>{if(currentView==='member')memberPointerDown=true;});
