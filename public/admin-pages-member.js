...
function memberRow(row,view){
 ...
 return `<tr ${category?`class="member-category-row"`:''}>${memberEditable(view)?`<td><input type="checkbox" class="member-check" aria-label="${esc(title)} 선택" data-id="${esc(row.id)}" ${memberSelected.has(row.id)?'checked':''}></td>`:''}<td class="member-item">${!['news','products'].includes(view)&&row.imageThumb?`<img class="member-content-thumb" src="${esc(row.imageThumb)}" alt="등록 사진">`:""}${category?(view==='products'?`<span class="member-game-icon" aria-label="${esc(category)}" title="${esc(category)}">${memberCategoryIcon(category,view)}</span>`:`<span class="member-category-label">${memberCategoryIcon(category,view)}${esc(memberStatus[category]||category)}</span>`):''}<strong>${esc(title)}</strong><div class="small-note">${esc(fmtTime(row.updatedAt||row.at||row.createdAt))}</div><span class="member-badge">${esc(memberState(row,view))}</span></td><td class="member-detail">${meta?`<div class="member-meta">${esc(meta)}</div>`:''}${memberDescription(body)}${view==='posts'&&row.quote?memberQuotePreview(row.quote):''}</td>${view==='posts'?`<td class="member-views">${Number(row.views||0).toLocaleString('ko-KR')}</td>`:''}<td><div class="actions">${actions||'—'}</div></td></tr>`;
}
