...
async function renderRecovery() {
  ...
  </div>${roleIsAdmin()?"<div class=\"toolbar\"><button id=\"queue-policy-save\" class=\"primary\">저장 정책</button></div>":''}<div class="warning-box">대기열은 앱 기기별 입력 순서를 보존합니다. 재전송에는 동일 요청 식별자를 사용해 WinSockServer RequestCache의 중복 처리 방지를 유지합니다.</div></div></div>
