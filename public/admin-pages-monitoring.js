...
async function renderDangerZone() {
  ...
  content.innerHTML = `<div class="danger-banner"><strong>!! DANGER ZONE !!</strong><span>위험 작업은 영향 범위를 확인한 뒤 Web 모달에서 한 번 더 승인합니다. 별도 확인 문구 입력은 사용하지 않습니다.</span></div><div class="danger-grid">
    ...
    <div class="section-card danger-card"><div class="section-head"><h3>All History Clean</h3><span class="small-note">ACTIVE DATA PRESERVED</span></div><div class="section-body"><p class="muted">완료된 Trace, Build, QR, Config, Daily, DLQ, Server/Client 접속, Notification, Audit 이력을 한 번에 정리합니다. 진행 중 데이터와 장비 등록은 보존됩니다.</p><button class="danger" data-history-clean="ALL">CLEAN ALL HISTORY</button></div></div>
  </div><div class="section-card danger-card registry-reset-card"><div class="section-head"><h3>Device Registry Reset</h3>${badge('ADMIN ONLY')}</div><div class="section-body"><p class="muted">등록된 모든 Server/Client ID, 1:1 연결, QR/PIN/Build, HMAC, 접속 이력과 DELETE 잠금을 초기화합니다. License 항목은 삭제하지 않고 기기 연결만 해제하며, 실행 직전에 자동 백업합니다. 모든 APK와 WinSockServer를 먼저 종료해야 합니다.</p><button id="danger-registry-reset" class="danger">RESET ALL DEVICES</button></div></div>`;
}
