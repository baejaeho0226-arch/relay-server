...
const DB_BAK_FILE = path.join(
    DATA_DIR,
    'relay-identities.bak.json'
);
...
const DEFAULT_MIN_PROTOCOL_VERSION =
    Number(
        process.env.MIN_PROTOCOL_VERSION ||
        1
    );

const DEFAULT_MIN_SERVER_VERSION =
    String(
        process.env.MIN_SERVER_VERSION ||
        '1.0.0'
    );

const DEFAULT_MIN_CLIENT_VERSION =
    String(
...
const CONFIRM_TOKEN_TTL_MS =
    60 * 1000;

const SERVER_KICK_BLOCK_MS =
    60 * 1000;

const CLIENT_KICK_BLOCK_MS =
    60 * 1000;
...
const MAX_CLIENTS_PER_SERVER =
    Math.max(
        1,
        Number(
            process.env.MAX_CLIENTS_PER_SERVER ||
            100
        )
    );

const REQUEST_HISTORY_TIMEOUT_MS =
    10 * 60 * 1000;
