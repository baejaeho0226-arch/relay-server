...
for(const source of ['ApkWinSock.Member.NewsShop.inc','ApkWinSock.Member.MyPage.inc','ApkWinSock.Member.Menu.inc','ApkWinSock.Member.Flow.inc']){
 ...
 if(source==='ApkWinSock.Member.NewsShop.inc')assert.ok(read(source).includes("HubNumber(Item,'views')"),source);
...
for(const key of ['Names','Actions','Groups','Icons']){
 ...
 assert.ok(match,key);assert.equal([...match[1].matchAll(/'[^']*'/g)].length,15,key);
}
...
console.log('FIX18 UI SOURCE PASS: fields/signatures, 84 SVG sources, native click/drag guards, bounded busy state, signed automatic refresh, menu arrays, feed and article-detail views, QR charge UI, game purchases, rectangular bars and no press effects');
...
for(const icon of ['heart','bookmark'])assert.notEqual(read('Svg/'+icon+'-filled.svg'),read('Svg/'+icon+'-outline.svg'));
